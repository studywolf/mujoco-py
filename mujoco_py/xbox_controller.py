# Core script from Martin O'Hanlon, used under MIT license
# git@github.com:martinohanlon/XboxController.git
...
class XboxController():#threading.Thread):
    ...
    def _start(self):
        ...
        for event in pygame.event.get():
            ...
            if event.type == JOYAXISMOTION:
                ...
                # check if pygame code is in our mapping to xbox
                if event.axis in self.AXISCONTROLMAP:
                    # map pygame code to xbox
                    ax_conv = self.AXISCONTROLMAP[event.axis]
                    # if the right thumbstick is hit, get both xy
                    if (ax_conv == self.XboxControls.RTHUMBX
                            or ax_conv == self.XboxControls.RTHUMBY):
                        axis_ids = [self.XboxControls.RTHUMBX,
                                    self.XboxControls.RTHUMBY]

                    # if the left thumbstick is hit, get both xy
                    elif (ax_conv == self.XboxControls.LTHUMBX
                            or ax_conv == self.XboxControls.LTHUMBY):
                        axis_ids = [self.XboxControls.LTHUMBX,
                                    self.XboxControls.LTHUMBY]
                    # how did we get here?
                    # else:
                    #     print('axis converted to xbox: ', ax_conv)
                    #     print('lthumbx: ', self.XboxControls.LTHUMBX)
                    #     print('lthumby: ', self.XboxControls.LTHUMBY)
                    #     print('rthumbx: ', self.XboxControls.RTHUMBX)
                    #     print('rthumby: ', self.XboxControls.RTHUMBY)

                    val = []
                    #print('hit key: ', ax_conv)
                    for ax in axis_ids:
                        val.append(self.joy.get_axis(ax))
                        # print('ax: ', ax)
                        # print('val: ', self.joy.get_axis(ax))
                    val = np.array(val)
                    #print('val array: ', val)
                    ...
                    self.updateControlValue(self.AXISCONTROLMAP[event.axis],
                                            self._sortOutAxisValue(val, yAxis),
                                            thumbstick=True)
    ...
    def updateControlValue(self, control, value, thumbstick=False):
        #if the value has changed update it and call the callbacks
        if thumbstick:
            if (control == self.XboxControls.RTHUMBX
                    or control == self.XboxControls.RTHUMBY):
                controls = [self.XboxControls.RTHUMBX,
                            self.XboxControls.RTHUMBY]
            elif (control == self.XboxControls.LTHUMBX
                    or control == self.XboxControls.LTHUMBY):
                controls = [self.XboxControls.LTHUMBX,
                            self.XboxControls.LTHUMBY]

            # if (self.controlValues[controls[0]] != value[0]
            #         or self.controlValues[controls[1]] != value[1]):
            self.controlValues[controls[0]] = value[0]
            self.controlValues[controls[1]] = value[1]
            self.doCallBacks(control, value)
        elif self.controlValues[control] != value:
    ...
    def _sortOutAxisValue(self, value, yAxis = False):
        #invert yAxis
        if yAxis and self.invertYAxis: value[1] = value[1] * -1
        #scale the value
        value = value * self.scale
        #apply the deadzone
        if value[0] < self.upperDeadzone and value[0] > self.lowerDeadzone: value[0] = 0
        if value[1] < self.upperDeadzone and value[1] > self.lowerDeadzone: value[1] = 0
        return value
